...
class DQNLocalPlannerNode(Node):
    def __init__(self):
        ...
        self.last_scan = None
        self.get_logger().info("DQN Local Planner Initialized.")

    def load_dqn_model(self):
        path = os.path.expanduser('~/MRS-2/dqn_model.pth')
        if os.path.exists(path):
            try:
                self.model.load_state_dict(torch.load(path))
                self.model.eval()
                self.get_logger().info("Loaded PyTorch DQN Model weights successfully.")
